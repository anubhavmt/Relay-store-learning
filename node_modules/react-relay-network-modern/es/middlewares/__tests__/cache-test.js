...
describe('middlewares/cache', () => {
  ...
  it('check `size` option', async () => {
    ...
    expect(fetchMock.calls('/graphql')).toHaveLength(1); // data from cache

    const res2 = await mockReq('FirstQuery').execute(rnl);
    expect(res2.data).toBe('PAYLOAD');
    expect(fetchMock.calls('/graphql')).toHaveLength(1); // data from fetch

    const res3 = await mockReq('SecondQuery').execute(rnl);
    expect(res3.data).toBe('PAYLOAD');
    expect(fetchMock.calls('/graphql')).toHaveLength(2); // data from cache

    const res4 = await mockReq('SecondQuery').execute(rnl);
    expect(res4.data).toBe('PAYLOAD');
    expect(fetchMock.calls('/graphql')).toHaveLength(2); // data from fetch

    const res5 = await mockReq('ThirdQuery').execute(rnl);
    expect(res5.data).toBe('PAYLOAD');
    expect(fetchMock.calls('/graphql')).toHaveLength(3); // first request should be removed from cache, cause size = 2
  ...
  it('do not use cache for mutations', async () => {
    ...
    const rnl = new RelayNetworkLayer([cacheMiddleware()]); // data from fetch
  ...
  it('do not use cache for responses with errors', async () => {
    ...
    const rnl = new RelayNetworkLayer([cacheMiddleware()]); // try fetch

    await expect(mockReq('FirstQuery').execute(rnl)).rejects.toThrow();
    expect(fetchMock.calls('/graphql')).toHaveLength(1); // try fetch again
  ...
  it('updates ttl on get if updateTTLOnGet is set', async () => {
    ...
    expect(res1.data).toBe('PAYLOAD');
    expect(fetchMock.calls('/graphql')).toHaveLength(1);
    await sleep(20); // data from cache

    const res2 = await mockReq('FirstQuery').execute(rnl);
    expect(res2.data).toBe('PAYLOAD');
    expect(fetchMock.calls('/graphql')).toHaveLength(1);
    await sleep(20); // data from cache 40ms after 30ms initial cache
